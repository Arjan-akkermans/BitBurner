/** @param {NS} ns
 * Intended to be used to purchase hacknet which is then used to boost a server to min security and max money, so we can spend some money here
...
export async function upgradeHacknet(ns: NS) {
    ...
    let counter = 0;
    while (ns.hacknet.getPurchaseNodeCost() < ns.getServerMoneyAvailable("home") && ns.hacknet.getPurchaseNodeCost() < 10e15 && ns.hacknet.numNodes() < ns.hacknet.maxNumNodes() && counter < 100) {
        counter++
    ...
    // if all upgrades are bought, then also spend some money on caches (Doesnt give profit, but needed to store )
    for (let i = 0; i < ns.hacknet.numNodes(); i++) {
        while (ns.hacknet.getCacheUpgradeCost(i, 1) <= 1e14) {
            if (ns.hacknet.getCacheUpgradeCost(i, 1) <= ns.getServerMoneyAvailable('home')) {
                moneySpend += ns.hacknet.getCacheUpgradeCost(i, 1)
